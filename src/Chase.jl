...
export ED, chase, chase_crel, chase_step, to_c_rel, from_c_rel, crel_type,
       egd, tgd, collage, pres_to_eds, extend_morphism

using Catlab.Theories, Catlab.Present
...
import Catlab.CategoricalAlgebra.Limits: universal
...
function crel_type(S::Presentation, n::Symbol)
  ...
  return AnonACSetType(pres#=, index=edges=#)
  #=expr = CSetDataStructures.struct_acset(name_, StructACSet, pres, index=edges)
  eval(expr)
  return eval(name_)=#
end
